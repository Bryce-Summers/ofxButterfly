...
/*
 * Butterfly subdivides once.
 */
ofMesh ofxButterfly::subdivide(ofMesh mesh)
{
	return ofxButterfly::subdivide(mesh, 1);
}
...
{
	gfx::WingedEdge WE_Output = new gfx::WingedEdge();
	...
	for(int i = 0; i < len_vertices; i++)
	{
		...
		dataVertices.std::vector::push_back(dataVert);
	...
	for(int i = 0; i < len_indexes; i+=3)
	{
		...
		v1 = dataVertices.std::vector::at(i1);
		v2 = dataVertices.std::vector::at(i2);
		v3 = dataVertices.std::vector::at(i3);
...
ofMesh fromWingedEdge(gfx::WingedEdge WE, std::map<gfx::Vertex, int> index_map)
{
	...
	for(std::map<gfx::Vertex, std::set<gfx::Edge>>::iterator iter = vertMap.begin(); iter != vertMap.end(); ++iter)
	{
		...
		if(index_map.find(v) == std::map<gfx::Vertex, int>::end)
		{
			index_map[v] = index;
			index++;
		}
	...
	for(std::map<gfx::Face, std::set<gfx::Edge>>::iterator iter = faceMap.begin(); iter != faceMap.end(); ++iter)
	{
		...
		int i1, i2, i3;
		i1 = index_map.std::map::find(v1) -> second;
		i2 = index_map.std::map::find(v2) -> second;
		i3 = index_map.std::map::find(v3) -> second;
